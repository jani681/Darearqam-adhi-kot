...
function App() {
  ...
  const handleLogin = async () => {
    ...
    if (!snap.empty) {
      ...
      setCurrentUser({ id: snap.docs[0].id, ...staffData });
      setUserRole('teacher');
      setIsLoggedIn(true);
      setView('sel_att'); // Teacher direct attendance par jayega
    } else {
  ...
  if (!isLoggedIn) return (
    <div style={{ background:'#1a4a8e', height:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', color:'white', padding:'20px' }}>
      ...
      <input type="password" placeholder="Enter Password" value={passInput} onChange={(e)=>setPassInput(e.target.value)} style={{padding:'15px', borderRadius:'10px', border:'none', width:'100%', maxWidth:'300px', textAlign:'center', color:'#333'}} />
      ...
      <p style={{marginTop:'15px', fontSize:'11px'}}>{status}</p>
    </div>
  );
    ...
    <div style={{ background: '#f4f7f9', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: '#1a4a8e', padding: '15px 10px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
          ...
          <button onClick={() => {setIsLoggedIn(false); setPassInput('');}} style={getNavStyle('out')}>🚪 Out</button>
        </div>
      </div>

      <div style={{ padding: '15px', maxWidth: '600px', margin: 'auto' }}>
        
        {/* DASHBOARD WITH FIXED BORDERS */}
        {view === 'dashboard' && (
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
            {CLASSES.map(c => (
              <div key={c} onClick={async () => {
                ...
                setView('view');
              }} style={{...cardStyle, borderLeft: '5px solid #1a4a8e', cursor:'pointer'}}>
                <b style={{color:'#1a4a8e', fontSize:'14px'}}>{c}</b>
        ...
        {view === 'view' && (
          <div>
            <div style={{display:'flex', gap:'5px', marginBottom:'10px'}}>
               <button onClick={()=>setView('dashboard')} style={{padding:'10px', background:'#666', color:'white', border:'none', borderRadius:'8px'}}>Back</button>
               <input placeholder="🔍 Search Name..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} style={{...inputStyle, margin:0}} />
            </div>
            {records.filter(r => r.student_name.toLowerCase().includes(searchTerm.toLowerCase())).map(r => (
              <div key={r.id} style={{...cardStyle, borderLeft: r.fee_status === 'Paid' ? '5px solid #2ecc71' : '5px solid #e74c3c'}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                  <div>
                    <b>{r.student_name}</b> <br/>
                    <small>Roll: {r.roll_number} | Dues: RS {r.total_dues}</small>
                  </div>
                  <div style={{display:'flex', gap:'5px'}}>
                    <a href={`https://wa.me/${r.parent_whatsapp}`} target="_blank" style={{background:'#25D366', color:'white', padding:'8px', borderRadius:'5px', textDecoration:'none', fontSize:'12px'}}>WA</a>
                    <button onClick={() => { setEditingStudent(r); setName(r.student_name); setRollNo(r.roll_number); setWhatsapp(r.parent_whatsapp); setBaseFee(r.base_fee); setArrears(r.arrears); setView('add'); }} style={{background:'#f39c12', color:'white', border:'none', padding:'8px', borderRadius:'5px'}}>Edit</button>
                    <button onClick={async () => { if(window.confirm("Delete?")) { await deleteDoc(doc(db, "ali_campus_records", r.id)); setView('dashboard'); } }} style={{background:'#e74c3c', color:'white', border:'none', padding:'8px', borderRadius:'5px'}}>Del</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {view === 'staff_list' && (
          <div>
            <div style={cardStyle}>
              ...
              <input placeholder="Monthly Salary" value={sSalary} onChange={(e)=>setSSalary(e.target.value)} style={inputStyle} />
              ...
              <button onClick={async () => {
                ...
                setSName(''); setSRole(''); setSSalary(''); setSPass(''); setView('dashboard');
              }} style={actionBtn}>Add Staff Member</button>
            </div>
            {staffRecords.map(s => (
              <div key={s.id} style={{...cardStyle, borderLeft: '5px solid #1a4a8e'}}>
                <b>{s.name}</b> ({s.role}) <br/> 
                <small>Salary: {s.salary} | Password: {s.password}</small>
              </div>
            ))}
          </div>
        )}
        ...
        {(view === 'sel_att' || view === 'sel_view') && (
          <div style={cardStyle}>
            ...
            <button onClick={async () => {
                ...
                const snap = await getDocs(q);
                setRecords(snap.docs.map(d => ({ id: d.id, ...d.data() })));
                setView(view === 'sel_att' ? 'attendance' : 'view');
            }} style={actionBtn}>Open Class</button>
          </div>
        )}
          ...
          <div>
            ...
            {records.map(r => (
              <div key={r.id} style={{...cardStyle, display:'flex', justifyContent:'space-between', background: attendance[r.student_name] === 'P' ? '#eaffea' : attendance[r.student_name] === 'A' ? '#ffeaea' : '#fff'}}>
                ...
                <div>
                  <button onClick={() => setAttendance({...attendance, [r.student_name]: 'P'})} style={{background: attendance[r.student_name] === 'P' ? 'green' : '#ccc', color:'#fff', border:'none', padding:'8px 12px', borderRadius:'5px'}}>P</button>
                  <button onClick={() => setAttendance({...attendance, [r.student_name]: 'A'})} style={{background: attendance[r.student_name] === 'A' ? 'red' : '#ccc', color:'#fff', border:'none', padding:'8px 12px', borderRadius:'5px', marginLeft:'5px'}}>A</button>
                </div>
              </div>
            ))}
            <button onClick={async () => {
              if(Object.keys(attendance).length === 0) return alert("Mark attendance first!");
              await addDoc(collection(db, "daily_attendance"), { class: filterClass, date: today, attendance_data: attendance, timestamp: serverTimestamp(), marked_by: userRole === 'admin' ? 'Admin' : currentUser.name });
              setAttendance({}); setView('dashboard');
            }} style={actionBtn}>Save Daily Attendance</button>
          </div>
        )}
        ...
        {view === 'add' && (
          <div style={cardStyle}>
            <h3>{editingStudent ? 'Update Student' : 'Student Admission'}</h3>
...
const cardStyle = { background: '#fff', padding: '15px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginBottom: '10px' };
const inputStyle = { width: '100%', padding: '12px', margin: '8px 0', borderRadius: '8px', border: '1px solid #ddd', boxSizing: 'border-box', fontSize:'14px' };
const actionBtn = { width: '100%', padding: '15px', background: '#1a4a8e', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', marginTop: '10px', fontSize:'16px' };
