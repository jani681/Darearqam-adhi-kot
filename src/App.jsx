...
function App() {
  ...
  // Search Logic: Naam ya Roll No dono par kaam karega
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#f4f7f9', minHeight: '100vh' }}>
      <div style={{ backgroundColor: '#1a4a8e', color: 'white', padding: '25px 10px', textAlign: 'center', position: 'relative' }}>
        <button onClick={() => { setIsLoggedIn(false); setPassInput(''); }} style={{ position: 'absolute', top: '10px', right: '10px', padding: '5px 12px', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '5px', fontSize: '10px', fontWeight: 'bold' }}>LOGOUT</button>
      ...
      <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto' }}>
        ...
        {(view === 'view' || view === 'attendance') && (
          <div style={{marginBottom: '15px'}}>
            <input 
              ...
              style={{...inputStyle, border: '2px solid #1a4a8e', padding: '12px'}}
            />
          </div>
        )}

        {view === 'dashboard' && <div style={{textAlign: 'center'}}><h3>Management Dashboard</h3></div>}

        {/* Directory View with Search */}
        {view === 'view' && (
          <div style={cardStyle}>
            <h3>Student List: {filterClass}</h3>
            {filteredRecords.length > 0 ? filteredRecords.map(r => (
              <div key={r.id} style={{padding: '10px', borderBottom: '1px solid #eee'}}>
                <strong>{r.student_name}</strong> <span style={{fontSize: '12px', color: '#666'}}>(Roll: {r.roll_number})</span>
              </div>
            )) : <p>No student matches your search.</p>}
          </div>
        )}

        {/* Attendance View with Search */}
        {view === 'attendance' && (
          <div style={cardStyle}>
        ...
        {/* Rest of the views (Admission, Selection, History) remain same */}
        {view === 'add' && (
          <div style={cardStyle}>
            <h3>New Admission</h3>
          ...
          <div>
            ...
            {history.map(h => (
              <div key={h.id} style={{...cardStyle, marginBottom: '10px'}}>
                <div style={{fontWeight: 'bold'}}>{h.date} | {h.class}</div>
                <div style={{fontSize: '12px', marginTop: '5px'}}>
                  {Object.entries(h.attendance_data).map(([n, s]) => <span key={n} style={{marginRight: '10px'}}>{n}: <b>{s}</b></span>)}
                </div>
              </div>
            ))}
          </div>
...
const cardStyle = { background: 'white', padding: '15px', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' };
