...
function App() {
  ...
  // 1. History Fetch Function (Robust Version)
  const fetchHistory = async () => {
    ...
    try {
      ...
      console.log("History Data:", historyData); // Debugging ke liye
  ...
  if (!isLoggedIn) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#1a4a8e', color: 'white' }}>
        <h3>Ali Campus Portal</h3>
        <div style={{ background: 'white', padding: '20px', borderRadius: '10px', color: 'black' }}>
          <input type="password" placeholder="Password" value={passInput} onChange={(e)=>setPassInput(e.target.value)} style={{padding:'10px', marginBottom:'10px', width:'200px'}} />
          <br/>
          <button onClick={handleLogin} style={{width:'100%', padding:'10px', background:'#1a4a8e', color:'white', border:'none'}}>Login</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#f4f7f9', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto' }}>
        ...
        {/* Class Selection for Attendance/Directory */}
        ...
        {view === 'attendance' && (
          <div style={cardStyle}>
            ...
            {records.map(r => (
              <div key={r.id} style={{display:'flex', justifyContent:'space-between', padding:'10px 0'}}>
            ...
            <button onClick={submitAttendance} style={actionBtn}>Save Daily Attendance</button>
          </div>
        )}
      </div>
    </div>
