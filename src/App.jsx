...
function App() {
  ...
  if (!isLoggedIn) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100vh', backgroundColor:'#1a4a8e', color:'white' }}>
      ...
      <button onClick={() => passInput === ADMIN_PASSWORD ? setIsLoggedIn(true) : alert("Wrong!")} style={{marginTop:'10px', padding:'10px 20px', borderRadius:'5px'}}>Login</button>
    </div>
  );
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#f4f7f9', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto' }}>
        ...
        {/* DIRECTORY VIEW - ALL BUTTONS INTEGRATED */}
            ...
            {records.filter(r => r.student_name?.toLowerCase().includes(searchTerm.toLowerCase())).map(r => (
              <div key={r.id} style={cardStyle}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                  <div>
                    <b style={{fontSize:'16px'}}>{r.student_name}</b> <br/>
                ...
                <div style={{display:'flex', gap:'5px', marginTop:'10px'}}>
                  ...
                  <button onClick={() => handleDelete(r.id)} style={{flex:1, padding:'6px', background:'#fff0f0', border:'1px solid #ffcccc', color:'red', borderRadius:'5px', fontSize:'11px'}}>Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {editingStudent && (
          <div style={cardStyle}>
            ...
            <button onClick={handleUpdate} style={actionBtn}>Update Student</button>
            <button onClick={() => setEditingStudent(null)} style={{width:'100%', marginTop:'5px', background:'#ccc', border:'none', padding:'10px'}}>Cancel</button>
          </div>
        )}
        ...
        {view === 'add' && (
          <div style={cardStyle}>
            ...
            <button onClick={async () => {
               await addDoc(collection(db, "ali_campus_records"), { student_name: name, roll_number: rollNo, parent_whatsapp: whatsapp, class: selectedClass, fee_status: 'Unpaid', created_at: serverTimestamp() });
               setView('dashboard');
            }} style={actionBtn}>Save Admission</button>
          </div>
        )}
        ...
        {view === 'dashboard' && (
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
            {CLASSES.map(c => <div key={c} style={cardStyle}><small>{c}</small><div style={{fontSize:'18px', fontWeight:'bold'}}>{classStats[c] || 0}</div></div>)}
          </div>
        )}
